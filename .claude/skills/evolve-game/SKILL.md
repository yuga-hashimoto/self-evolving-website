...
**推奨ゲームタイプ:**
- シンプルなパズルゲーム（マインスイーパー、2048、マッチ3など）
- アクションゲーム（ジャンプゲーム、フラッピーバード風）
- 積み上げ・落ちものゲーム（テトリス風、スタッキング）
...
**1. Canvas レスポンシブ対応（必須）**
- ❌ `const CANVAS_WIDTH = 300` のような固定値は絶対禁止
- ✅ `window.innerWidth` を使って画面幅に応じて動的にサイズ設定
- ✅ 画面リサイズにも対応すること

**2. タッチ操作（必須）**
- ✅ タッチイベントは連続的に処理すること（ボタン長押し対応）
- ✅ スワイプ判定は画面サイズに応じて調整（画面幅の10%程度を目安）
- ✅ ジャンプ系ゲームは、デバイス傾き検知または連続タッチ入力に対応

**3. その他の推奨事項**
- バイブレーションフィードバック（`navigator.vibrate`）
- フォントサイズ・タイルサイズの動的調整
- 小さい画面（375px幅）でも見やすいUI

**必須チェックリスト**

実装完了前に、必ず以下を確認してください：

- [ ] `const CANVAS_WIDTH = 数値` のような固定値を使っていないか？
- [ ] `window.innerWidth` を使って画面幅を取得しているか？
- [ ] タッチ操作は連続的に処理されるか？
- [ ] スワイプ判定は画面サイズに応じているか？
- [ ] 小さい画面でも文字・タイルは見えるか？
...
**良い例：**
- ✅ 「直帰率が72%と高いため、最初の30秒で楽しめるシンプルなゲームを追加し、滞在時間を延ばす」
- ✅ 「セッション時間が平均45秒と短い原因を分析。操作が難しすぎたため、チュートリアルとヒント機能を追加してユーザー体験を改善」
