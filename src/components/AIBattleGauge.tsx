...
export default function AIBattleGauge() {
  ...
  const [mimoScore, setMimoScore] = useState(55);
  const [grokScore, setGrokScore] = useState(45);
  ...
  useEffect(() => {
    setGrokScore(parseFloat((100 - mimoScore).toFixed(1)));
  }, [mimoScore]);
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-6 bg-gray-900/50 backdrop-blur-sm rounded-xl border border-gray-800 shadow-2xl my-8 relative overflow-hidden group">
      ...
      <div className="relative h-10 bg-gray-800/50 rounded-full overflow-hidden border border-white/5 backdrop-blur-md shadow-inner">
        ...
        {/* Grok Bar (Right) - We simulate this by just having Mimo cover the left side, but let's make it explicit for the gradient */}
        ...
        <div className="absolute inset-0 flex justify-between items-center px-6 font-bold text-white text-base z-10 drop-shadow-md">
          <span className="flex items-center gap-2">
            <span className="text-purple-100 drop-shadow-[0_0_8px_rgba(168,85,247,0.8)]">Mimo</span>
            <span className="tabular-nums font-mono text-sm opacity-90">{mimoScore.toFixed(1)}%</span>
          </span>
          <span className="flex items-center gap-2">
            <span className="tabular-nums font-mono text-sm opacity-90">{grokScore.toFixed(1)}%</span>
            <span className="text-blue-100 drop-shadow-[0_0_8px_rgba(59,130,246,0.8)]">Grok</span>
          </span>
        </div>
