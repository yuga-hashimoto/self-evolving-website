...
export const FortuneCookie = () => {
  ...
  const [luckyNumbers, setLuckyNumbers] = useState<{n1: number, n2: number, hex: string} | null>(null);
  ...
  const openCookie = () => {
    if (!isOpen) {
      ...
      setLuckyNumbers({
        ...
        hex: Math.floor(Math.random() * 255).toString(16).toUpperCase()
      });
      setIsOpen(true);
    } else {
      setIsOpen(false);
          ...
          >
            ...
            {luckyNumbers && (
              <p className="text-xs text-gray-500 font-mono uppercase tracking-widest border-t border-gray-200 pt-2 mt-2">
                Lucky Numbers: {luckyNumbers.n1} • {luckyNumbers.n2} • 0x{luckyNumbers.hex}
              </p>
            )}
